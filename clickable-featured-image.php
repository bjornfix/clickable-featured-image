<?php
/*
Plugin Name: Clickable Featured Image
Plugin URI: https://wordpress.org/plugins/clickable-featured-image/
Description: A plugin that replaces the featured image in a post or page with one that is clickable if there is a featured image and links to the full size image.
Version: 1.0.13
Author: Devenia
Author URI: https://devenia.com/
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
*/

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

function cfi_mark_post_featured_image_block($parsed_block) {
    if (!empty($parsed_block['blockName']) && $parsed_block['blockName'] === 'core/post-featured-image') {
        $GLOBALS['cfi_post_featured_image_block'] = $parsed_block;
    }
    return $parsed_block;
}
add_filter('render_block_data', 'cfi_mark_post_featured_image_block', 10, 1);

function cfi_add_mwl_img_class($html, $attachment_id) {
    if (empty($attachment_id) || preg_match('/wp-image-' . intval($attachment_id) . '\b/', $html)) {
        return $html;
    }

    if (preg_match('/class\s*=\s*["\']([^"\']*)["\']/i', $html)) {
        $updated = preg_replace(
            '/class\s*=\s*(["\'])([^"\']*)(\1)/i',
            'class=$1$2 wp-image-' . intval($attachment_id) . '$3',
            $html,
            1
        );
        if (!empty($updated)) {
            return $updated;
        }
    }

    if (preg_match('/<img\b/i', $html)) {
        $updated = preg_replace('/<img\b/i', '<img class="wp-image-' . intval($attachment_id) . '"', $html, 1);
        if (!empty($updated)) {
            return $updated;
        }
    }

    return $html;
}

function cfi_wrap_featured_media($html, $anchor_open, $anchor_close) {
    if (preg_match('/<a\\s/i', $html)) {
        return $html;
    }

    if (preg_match('/<picture\\b/i', $html)) {
        $wrapped = preg_replace('/(<picture\\b[^>]*>.*?<\\/picture>)/is', $anchor_open . '$1' . $anchor_close, $html, 1);
        if (!empty($wrapped)) {
            return $wrapped;
        }
    }

    if (preg_match('/<img\\b/i', $html)) {
        $wrapped = preg_replace('/(<img\\b[^>]*>)/i', $anchor_open . '$1' . $anchor_close, $html, 1);
        if (!empty($wrapped)) {
            return $wrapped;
        }
    }

    return $anchor_open . $html . $anchor_close;
}

function cfi_remove_auto_sizes($html) {
    if (empty($html) || stripos($html, ' sizes=') === false) {
        return $html;
    }

    $updated = preg_replace("/\\s+sizes=([\"'])auto,\\s*/i", ' sizes=$1', $html, 1);

    return !empty($updated) ? $updated : $html;
}

function cfi_clickable_featured_image($html, $post_id, $post_thumbnail_id) {
    $image_data = wp_get_attachment_image_src($post_thumbnail_id, 'full');

    // Check if $image_data is false, and if so, return the original $html.
    if ($image_data === false) {
        return $html;
    }

    $caption = get_the_title($post_thumbnail_id);
    $block_context = isset($GLOBALS['cfi_post_featured_image_block']) ? $GLOBALS['cfi_post_featured_image_block'] : null;
    $block_has_link = is_array($block_context) && !empty($block_context['attrs']['isLink']);
    $GLOBALS['cfi_post_featured_image_block'] = null;

    // Check if image is already wrapped in an anchor (e.g., by theme in query loops)
    if (preg_match('/<a\b/i', $html)) {
        return $html;
    }

    if (is_singular()) {
        $html = cfi_add_mwl_img_class($html, $post_thumbnail_id);
        $anchor_open = '<a href="' . esc_url($image_data[0]) . '" data-caption="' . esc_attr($caption) . '" class="cfi-featured-image-link">';
        $anchor_close = '</a>';
        $html = cfi_wrap_featured_media($html, $anchor_open, $anchor_close);
    } else {
        if ($block_has_link) {
            return $html;
        }
        $html = cfi_remove_auto_sizes($html);
        $post_url = get_permalink($post_id);
        $anchor_open = '<a href="' . esc_url($post_url) . '" class="cfi-featured-image-link">';
        $anchor_close = '</a>';
        $html = cfi_wrap_featured_media($html, $anchor_open, $anchor_close);
    }

    return $html;
}

add_filter('post_thumbnail_html', 'cfi_clickable_featured_image', 10, 3);

function cfi_enqueue_styles() {
    if (!is_singular()) {
        wp_register_style('cfi-style', false, array(), '1.0.13');
        wp_enqueue_style('cfi-style');
        $thumbnail_width = absint(get_option('thumbnail_size_w', 150));
        $thumbnail_height = absint(get_option('thumbnail_size_h', 150));
        wp_add_inline_style('cfi-style', sprintf('
            .post-image .cfi-featured-image-link,
            .cfi-featured-image-link {
                display: inline-block;
                width: auto;
                max-width: 100%%;
            }
            .post-image .cfi-featured-image-link img,
            .cfi-featured-image-link img {
                display: block;
                width: auto !important;
                max-width: 100%%;
                height: auto !important;
            }
            .post-image .cfi-featured-image-link img.attachment-thumbnail,
            .cfi-featured-image-link img.attachment-thumbnail {
                width: %1$dpx !important;
                height: %2$dpx !important;
                object-fit: cover;
            }
        ', $thumbnail_width, $thumbnail_height));
    }
}
add_action('wp_enqueue_scripts', 'cfi_enqueue_styles');

function cfi_enqueue_meow_lightbox_bridge() {
    if (!is_singular() || !has_post_thumbnail()) {
        return;
    }

    wp_register_script('cfi-meow-lightbox-bridge', false, array(), '1.0.12', true);
    wp_enqueue_script('cfi-meow-lightbox-bridge');
    wp_add_inline_script(
        'cfi-meow-lightbox-bridge',
        "(function(){\n" .
        "  var selectors = ['.featured-image', '.page-header-image-single', '.page-header-image', '.post-image'];\n" .
        "  var delay = 250;\n" .
        "  var maxAttempts = 12;\n" .
        "  function bindFeaturedImage(attempt){\n" .
        "    if (typeof window.renderMeowLightboxWithParentElement !== 'function') {\n" .
        "      if (attempt < maxAttempts) {\n" .
        "        window.setTimeout(function(){ bindFeaturedImage(attempt + 1); }, delay);\n" .
        "      }\n" .
        "      return;\n" .
        "    }\n" .
        "    selectors.forEach(function(selector){\n" .
        "      document.querySelectorAll(selector).forEach(function(container){\n" .
        "        var images = container.querySelectorAll('img');\n" .
        "        if (!images.length) {\n" .
        "          return;\n" .
        "        }\n" .
        "        images.forEach(function(image){\n" .
        "          image.removeAttribute('data-mwl-img-id');\n" .
        "        });\n" .
        "        if (typeof window.destroyFromMeowLightbox === 'function') {\n" .
        "          window.destroyFromMeowLightbox(container);\n" .
        "        }\n" .
        "        if (container.querySelector('img')) {\n" .
        "          window.renderMeowLightboxWithParentElement(container);\n" .
        "        }\n" .
        "      });\n" .
        "    });\n" .
        "  }\n" .
        "  function queueBind(){\n" .
        "    window.setTimeout(function(){ bindFeaturedImage(0); }, 0);\n" .
        "  }\n" .
        "  if (document.readyState === 'loading') {\n" .
        "    document.addEventListener('DOMContentLoaded', queueBind, { once: true });\n" .
        "  } else {\n" .
        "    queueBind();\n" .
        "  }\n" .
        "  window.addEventListener('load', queueBind, { once: true });\n" .
        "})();",
        'after'
    );
}
add_action('wp_enqueue_scripts', 'cfi_enqueue_meow_lightbox_bridge');

function cfi_enqueue_lightbox_assets() {
    if (is_singular() && has_post_thumbnail()) {
        add_filter('baguettebox_enqueue_assets', '__return_true');
    }
}
add_action('wp', 'cfi_enqueue_lightbox_assets');
