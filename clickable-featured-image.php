...
function cfi_clickable_featured_image($html, $post_id, $post_thumbnail_id) {
    ...
    // Skip if the image is already wrapped in a link (prevents nested <a> tags)
    ...
    if (is_singular()) {
        $html = '<a href="' . esc_url($image_data[0]) . '" data-caption="' . esc_attr($caption) . '" class="wp-block-image">' . $html . '</a>';
    } else {
        $post_url = get_permalink($post_id);
        $html = '<a href="' . esc_url($post_url) . '">' . $html . '</a>';
    }
